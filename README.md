...
### 1.1 Repository layout (five top-level directories)

Most of the project lives in five sibling directories.  Three of them
hold "CP/M disk contents" — collections of files meant to be uploaded
onto a CP/M floppy / disk image and used from inside CP/M itself.  One
holds the per-piece MIDI sources and converter outputs that feed into
those disks.  One is the Python tooling that supports the modern half
of the work.
...
- **`MIDI/`** — one subfolder per piece set, each containing the
  source `.mid` files, the converted `.MUS` files, the SUBMIT batch
  that plays them on PLAYZ80, and a per-disk `README.md` describing
  the music and the conversion settings used.  `MIDI/bach/` is the
  pristine Bach Central source corpus, organized by work type
  (`aof/`, `brandenb/`, `cellosui/`, `chorales/`, `gold/`, `invent/`,
  `organ/`, `partitas/`, `sinfon/`, `wtcbki/`, `wtcbkii/`, …) and
  kept around for re-conversion experiments.  Per-disk dev folders
  (e.g. `MIDI/CelloSuiteNumberOneinG/`,
  `MIDI/WellTemperedClavierBookOne_FourVoiceFugues/`,
  `MIDI/ArtOfFugue_FourVoicePieces/`) are the curated subsets that
  feed into `Disks/`.

- **`Disks/`** — committed archive of every CP/M disk image used
  with the player.  Each disk appears in two forms:
  - `Bach*.dsk` / `TETRIS-Music.dsk` / `music.dsk` — packed
    256-sector floppy images (compatible with fdcServer and Z80Pack);
    suitable for booting an emulator or writing to a uSD card for a
    real IMSAI 8080.
  - `Bach*.unpacked/` / `TETRIS-Music.unpacked/` / `music.unpacked/`
    — the same content as host-side folder trees that fdcServer can
    serve directly without repacking; numbered subfolders `0/..15/`
    are the CP/M USER areas.
  Each disk's `USER 0` folder also carries a CP/M-shaped
  `README.TXT` describing the music, files, conversion process,
  and contact info — generated by `Scripts/_gen_disk_readme.py`.
  See [`Disks/README.TXT`](Disks/README.TXT) for the top-level
  archive overview.
...
| `midi2mus.py` | MIDI -> `.MUS` converter.  Modes `1` / `2` / `3` / `4` / `2of3` (solo / invention / sinfonia faithful / SATB chorale / sinfonia-with-soprano-doubling).  Honors MIDI tempo changes (rubato) by varying the per-command `dur` byte.  Mode `1` has chord-moment detection that allocates up to 3 simultaneous notes across V1/V2/V3 and uses a `coverage * duration` heuristic to drop short trill ornaments instead of pinning two adjacent semitones together.  Mode `4` maps four monophonic MIDI voice tracks one-to-one onto V1..V4 with default timbres (sine, saw, square, sine) so every voice plays at its written pitch -- use for 4-part chorales and 4-voice fugues.  Flags: `--timbres V1,V2,V3,V4` overrides the mode's default timbre routing (e.g. `2,2,2,0` for all-triangle chiptune feel); `--merge` (mode `1` only) merges every non-drum track into one polyphonic source for multi-track arrangements. |
...
| `_gen_disk_readme.py` | Generate the CP/M-shaped `README.TXT` that lives inside each `Bach*.unpacked/0/` disk image (music description, file list, conversion process, D+7AIO ports, ASCII mixer schematic, fdcServer + repo links).  Writes the same content to both `Disks/<disk>.unpacked/0/README.TXT` (committed archive) and `D:/CPMEMU/disks/<disk>.unpacked/0/README.TXT` (live emulator) so they stay in sync.  Emits CRLF + Ctrl-Z directly — no separate `CPMFMT.PY` pass needed. |
...
│   ├── _gen_disk_readme.py    (writes per-disk README.TXT to both Disks/ and D:/CPMEMU/disks/)
│   └── README.md              (per-script reference)
├── MIDI/                      (MIDI-sourced piece sets; one disk image per subfolder)
│   ├── bach/                  (pristine Bach Central source corpus; subfolders aof/, brandenb/, cellosui/, chorales/, gold/, invent/, organ/, partitas/, sinfon/, wtcbki/, wtcbkii/, ...; kept for re-conversion)
...
│   ├── WellTemperedClavierBookOne_FourVoiceFugues/  (8 four-voice fugues from BWV 846-869; --voices 4)
│   ├── WellTemperedClavierBookTwo_PreludesAndFugues/ (21 prelude/fugue movements BWV 870-881; --voices 1 --merge; single-track polyphonic sources)
│   └── ArtOfFugue_FourVoicePieces/                  (12 four-voice movements from BWV 1080; --voices 4)
├── Disks/                     (committed archive of CP/M disk images)
│   ├── README.TXT             (archive overview -- what's in this folder)
│   ├── Bach*.dsk, music.dsk, TETRIS-Music.dsk        (packed 256-sector floppy images for fdcServer / Z80Pack)
│   └── Bach*.unpacked/, music.unpacked/, TETRIS-Music.unpacked/  (host-side folder trees; same content as the .dsk; each disk's 0/ subfolder is CP/M USER 0)
├── Voice_Tests/               (timbre tests: .MUS files)
...
`CELLO.SUB` / `INVENT.SUB` / `SINFONIA.SUB` / `WTCBKI.SUB` /
`WTCBKII.SUB` / `AOF.SUB` batch (plus a byte-identical `.QSB` copy
for the QPM convention) to play them back-to-back, and -- for the
cello and TETRIS sets -- a self-contained custom `VOICES.MUS` that
overrides slot 2 with a piece-appropriate wavetable.  The
ready-to-use disk images themselves live in [`Disks/`](Disks/) in
both packed (`.dsk`) and unpacked (`.unpacked/`) form; upload an
unpacked folder's `0/` contents verbatim to a CP/M disk (or mount
the `.unpacked/` directly with fdcServer) and `SUBMIT CELLO`
(or equivalent) plays the whole set with the right timbres.
